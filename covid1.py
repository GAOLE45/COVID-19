...
try:
    ...
    def draw_bar_confirm(date):
        ...
        ax.clear()
        # Plotting with worldwide time series data
        ...
        # Set coordinate axis and scale format
...
def world_map(day):
    ...
    data_geod = gp.GeoDataFrame(confirm_country1.loc[:, [date, 'Country/Region']])  ##Convert data to geopandas.DataFrame
    world_ = world.rename(index=str, columns={'name': 'Country/Region'})  # Modify column name
    da_merge = data_geod.merge(world_, how='left')  # Data merge
...
def line(region):
    if(region=='global'):
        plt.figure(figsize=(16, 9))
        # marker is to set the identification of each data point, markerfacecolor sets the color within the point
        ...
        plt.plot(range(0, len(confirm.columns.drop(['Province/State','Country/Region','Lat','Long','group']))), recover_con.drop(columns=['country', 'group']).cumsum().loc[last].values,
                 marker='o', markerfacecolor='#ffffff', linewidth=3)
        # Fill color, alpha is to set color transparency
...
beta_c = st.text_input('Infectivity coefficient',value=0.7)
gamma_c = st.text_input('Recovery factor', value=0.17)
...
def pred_long(Te,beta,gamma,T):
    ...
    N = 329227746
    # β = Infectivity coefficient
    beta = float(beta)
    # gamma = Recovery factor
    gamma = float(gamma)
    # Te = Incubation period
    Te = Te

    # T = Travel time
    T = T

    # INI = Array in initial state 
    ...
    # I_0 = The initial number of infected persons
    I_0 = US.values[len(US.values)-1]
    # E_0 = The initial number of lurkers
    E_0 = RES[:,1][len(RES[:,1])-1]
    # R_0 = Initial number of healers
    R_0 = US_re.values[len(US_re.values)-1]
    # S_0 = Initial number of susceptible
    S_0 = N - I_0 - E_0 - R_0
